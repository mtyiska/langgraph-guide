...
def run_review_loop(graph, config: dict, initial_input):
    """
    Drive the review loop: invoke the graph, handle each interrupt,
    inject human decisions, and resume until the graph completes.
    """
    # First invocation (or resumption with None input)
    try:
    ...
    while True:
        ...
        # We are paused before apply_or_skip — collect human decision
        if "apply_or_skip" in state.next:
            try:
                decision: ReviewDecision = present_change_for_review(graph, config)
            except SystemExit:
                # Human chose to quit — state already saved in SQLite
                return

            if decision is None:
                # No pending change — shouldn't happen, but resume anyway
                graph.invoke(None, config)
                continue

            # Inject the human's decision into state
            graph.update_state(
                config,
            ...
            # Unexpected interrupt — resume anyway
...
def main():
    ...
    # Determine session ID and whether this is a new or resumed session
    if args.session:
        ...
        if not state.values:
            ...
            initial_input = None  # continue from checkpoint
